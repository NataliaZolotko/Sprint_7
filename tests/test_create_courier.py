...
fake = Faker()
from data import DataCreateCourier, Url
...
class TestCreateCourier:
    ...
    def test_create_courier_success(self):
        data = DataCreateCourier.Create_Courier_Body()
        response = CreateCourierMethod.createcourier(data)
    ...
    def test_create_courier_with_existing_login_fails(self):
        ...
        response = CreateCourierMethod.createcourier(duplicate_data)
        assert response.status_code == 409 and response.json()["message"] == "Этот логин уже используется. Попробуйте другой."